...
static void NRF_Init(void)
{
	...
	/* 配置SPI2的SCK(PB13),MISO(PB14),MOSI(PB15)引脚 */
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13 | GPIO_Pin_14 | GPIO_Pin_15;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF_PP;
	GPIO_Init(GPIOB, &GPIO_InitStructure);

	/* 配置NRF的CE(PG8),CSN(PG7)引脚 */
...
void nrfInit(enum nrfMode model)
{
	...
	if (model == PRX_MODE)
	{
		writeReg(REG_CONFIG, 0x0f);  /* IRQ收发完成中断开启,16位CRC,PRX 0x0f	0x4f:保证在TX_DS后冲洗RX FIFO*/
		writeReg(REG_DYNPD, 0x03);   /* 使能RX_P0\1动态长度PLAYLOAD */
		writeReg(REG_FEATURE, 0x06); /* 使能动态长度PLAYLOAD、发送ACK PLAYLOAD */

		writeReg(REG_EN_AA, 0x03); /* 使能通道0\1的自动应答 */

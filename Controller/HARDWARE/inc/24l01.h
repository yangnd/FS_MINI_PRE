...
********************************************************************************/
/* 用户配置参数（默认） */
#define DEFAULT_ADDR			0x0110104334ULL
#define DEFAULT_CHANNEL 	 	40
#define DEFAULT_DATARATE  		DATA_RATE_2M
...
#define  NRF_SPI  			SPI2
#define  NRF_CE_H()			PGout(8) = 1 
#define  NRF_CE_L()			PGout(8) = 0 

#define  SPI2_CSN_H()		PGout(7) = 1 
#define  SPI2_CSN_L()		PGout(7) = 0 
...
//u8 nrf_sendPacketWaitACK(u8 *sendBuf, u8 len, u8 *ackBuf, u8 *acklen,u8 *rx_Pn);

/* 查询事件并接收数据包 */
nrfEvent_e nrf_checkEventandRxPacket(u8 *ackBuf, u8 *acklen);
//nrfEvent_e nrf_checkEventandRxPacket(u8 *ackBuf, u8 *acklen,u8 *rx_Pn);
...
//void nrf_setAddress(uint64_t address,uint64_t address_P1);
void nrf_setTXAddress(uint64_t address);
